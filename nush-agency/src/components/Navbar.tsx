...
export default function Navbar() {
  ...
  return <nav className="fixed top-0 left-0 right-0 z-50 glass border-b border-white/10">
    <div className="px-6 md:px-8 py-4 flex items-center justify-between gap-6">
      ...
      <a href="#kontakt" className="hidden sm:block bg-neon text-black px-5 py-3 text-xs md:text-sm font-black uppercase hover:bg-white hover:-translate-y-0.5 transition-all">Bezpłatna diagnoza</a>
    ...
    {open && <div className="xl:hidden border-t border-white/10 bg-[#050505] px-6 py-5 flex flex-col gap-5 text-sm font-bold uppercase tracking-widest">{links.map(([label, href]) => <a key={href} href={href} onClick={() => setOpen(false)} className="hover:text-neon">{label}</a>)}<a href="#kontakt" onClick={() => setOpen(false)} className="bg-neon text-black px-4 py-3 text-center">Bezpłatna diagnoza</a></div>}
  </nav>;
}
