...
export default function Navbar() {
  ...
  return <>
    <nav className="fixed top-0 left-0 right-0 z-50 glass border-b border-white/10">
      ...
      {open && <div className="xl:hidden border-t border-white/10 bg-[#050505] px-6 py-5 flex flex-col gap-5 text-sm font-bold uppercase tracking-widest">{links.map(([label, href]) => <a key={href} href={href} onClick={() => setOpen(false)} className="hover:text-neon">{label}</a>)}</div>}
    </nav>
